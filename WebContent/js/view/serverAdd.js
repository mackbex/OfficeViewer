$.ServerAdd = {
		...
		//	$.Common.ShowProgress("#cocard_progress","Waiting..","000000","0.7");
			...
			//Set globalization.
			$.ServerAdd.localeMsg = $.Common.Localize($.Lang, "data-i18n", params.LANG,"ServerAdd");

			$("#searchUserKeyword").attr('placeholder',$.ServerAdd.localeMsg["PLACEHOLDER_INPUT_USERINFO"]);
			//Set UI set
			$.ServerAdd.setUIColor();

			$.ServerAdd.initInputDateProc();


			$.ServerAdd.drawGridView();
			$.ServerAdd.viewer = $.ServerAdd.setImageViewer();
			// if(!$.Common.isBlank($.ServerAdd.params.APPR_NO)) {
			// 	$("#ipt_apprNo").val($.ServerAdd.params.APPR_NO);
			// 	$.ServerAdd.Search();
			// }
			...
			//Get slipKindList
			$.ServerAdd.initSlipKindUI();

			//Load Bookmark
			$.ServerAdd.initBookmarkModule();

		},
		initBookmarkModule : function(){

			if($.ServerAdd.params.USE_BOOKMARK) {
				if($.Common.GetBrowserVersion().ActingVersion > 9) {
					$.getMultiScripts([g_VIEW_BOOKMARK_URL, g_CANVAS_LIB, g_DRAW_BOOKMARK_WEB])
						...
						.done(function () {
							$.ServerAdd.isBookmarkLoaded = true;
						});
				}
			}
		},
		initSlipKindUI : function(){
			...
			$.when($.Operation.execute($.ServerAdd, "GET_SLIPKIND_LIST", $.ServerAdd.params)).then(function(res) {
				$.ServerAdd.addSlipKind(res);
			})
			.fail(function(error){
				$.Common.simpleAlert(null, $.ServerAdd.localeMsg.FAILED_LOAD_SLIPKIND, null);
			})
			.always(function(){
			...
			$("#fromDate").on("keyup", function(e){
				...
				if (e.keyCode === 13) {
					...
					if(val.length !== 10 || isNaN(date.getTime())) {
						$.Common.simpleToast($.ServerAdd.localeMsg.INPUT_VALID_DATE);
					}
					else {
		...
		PopupSearchUser : function() {
			$("#AREA_LAYER_POPUP").load(g_RootURL +"popupSearchUser.jsp", {LANG: $.ServerAdd.params.LANG}, function(response, status, xhr) {
		...
		Search : function (isDefaultSearch) {
			...
			$.ServerAdd.resetViewer();

			$.ServerAdd.changeCheckStatus(false);
			var params = $.ServerAdd.getSearchParams();
			if(params !== null) {
//
...
//
				$.when($.Operation.execute($.ServerAdd, "SEARCH_SLIP", params)).then(function(res) {
					$.ServerAdd.Push_SearchResultItem(res);
				})
				.fail(function(error){
					$.Common.simpleAlert(null, $.ServerAdd.localeMsg.FAILED_SEARCH, null);
				})
				.always(function(){
		...
		Push_SearchResultItem: function(res){
			...
			Object.keys(res).forEach(function(dataKey) {
				...
				var item = {
					...
					"used" : res[dataKey].SDOC_STEP === "0" || res[dataKey].SDOC_STEP === "1" ? $.ServerAdd.localeMsg.UNUSED : $.ServerAdd.localeMsg.USED,
					...
					"category" : res[dataKey].FOLDER  == "SLIPDOC" ? $.ServerAdd.localeMsg.CATEGORY_SLIP : $.ServerAdd.localeMsg.CATEGORY_ADDFILE,
					"isChecked":false
				}
		...
		// 	if(idx < 0) {
		// 		$.Common.simpleAlert(null, $.ServerAdd.localeMsg.ALERT_CHECK_SLIP, 0.3);
		// 	}
		// 	else {
		//
...
//
			...
			$.when($.Common.RunCommand(g_ActorCommand, "GET_SLIP_LIST", params)).then(function(res) {
				...
				if(res == null || resCnt <= 0) {
					...
					$.ServerAdd.setSlipPager(res);

					var version = $.Common.GetBrowserVersion().ActingVersion;

					if(version > 9 && $.ServerAdd.isBookmarkLoaded) {
						$.ServerAdd.displayCanvasOriginal(res[Object.keys(res)[0]]);
					}
					else {
						$.ServerAdd.displayImage(res[Object.keys(res)[0]]);
					}
			...
			.fail(function(error){
				$.Common.simpleToast(null, $.ServerAdd.localeMsg.FAILED_SEARCH, null);
		...
		displayCanvasOriginal : function(objData) {
			...
			$.Common.ShowProgress(".original_progress","Waiting..","000000","0");
			objData['IMG_DPI'] = $.ServerAdd.params.IMG_DPI;
			var canvas = null;
			...
			sbImgURL.append("&UserID="+$.ServerAdd.params.USER_ID);
			sbImgURL.append("&CorpNo="+$.ServerAdd.params.CORP_NO);

			elImage.src = sbImgURL.toString();

			bookmarkContext.init(canvas, $(elImage), objData, $.ServerAdd.params);
			$.when(bookmarkContext.drawTargetImage(".original_progress")).then(function(){
				...
				if("T" === $.ServerAdd.params.USE_BOOKMARK_DRAW) {
					if($.ServerAdd.drawContext === null) {
						$.ServerAdd.drawContext = Draw();
						var i18nMsgProps = $.Common.Localize($.Lang, "data-i18n", $.ServerAdd.params.LANG,"Draw");
						$.ServerAdd.drawContext.init(bookmarkContext, i18nMsgProps, "#colorPicker");
					}
					else {
						$.ServerAdd.drawContext.reset(bookmarkContext);
					}
				}
			});
			...
			if(Object.keys(obj).length > 1) {
				...
				var elSelect = $(document.createElement('select'))
					.unbind().bind('change', function(){
						...
						if(version > 9 && $.ServerAdd.isBookmarkLoaded) {
							$.ServerAdd.displayCanvasOriginal(obj[this.value]);
						}
						else {
							$.ServerAdd.displayImage(obj[this.value]);
						}
		...
		resetViewer : function() {
			if ($.ServerAdd.viewer != null) {
				$.ServerAdd.viewer.panzoom("reset", false);
			}
		...
		register : function(){
			...
			if(items.length <= 0) {
				$.Common.simpleToast($.ServerAdd.localeMsg.SELECT_SLIP);
				return;
			}

			$.Common.simpleConfirm(null,$.ServerAdd.localeMsg.CONFIRM_REGISTER_CHECKED, function() {

				var params = {
					CHECKED_LIST : JSON.stringify(items),//$.map(items,function(obj) { return obj.sdocNo})
					JDOC_NO : $.ServerAdd.params.JDOC_NO
				//	VALUE :  $.map(items,function(obj) { return obj.sdocNo})
				}

				$.when($.Common.RunCommand(g_ServerAddCommand, "UPLOAD_SLIP", params)).then(function (objRes) {
					if("T" === objRes.RESULT) {
						$.Common.simpleAlert(null, $.ServerAdd.localeMsg.ADD_SLIP_SUCCESS,null,function() {
							window.close();
							window.opener.$.Actor.pageSubmit();
						});
					}
					else {
						$.Common.simpleAlert(null, $.ServerAdd.localeMsg[objRes.MSG]);
					}
				...
				rowClick : function(args) {
					...
					if(args.item.sdocNo !== curKey) {
						if (args.item.folder === "SLIPDOC") {
							$.ServerAdd.resetViewer();
							$.ServerAdd.getImageData(args.item);
						}
						else {
									...
									$(document.createElement('label'))
										...
										.prepend([
											$(document.createElement('input'))
												...
												.on("change", function () {
													$.ServerAdd.changeCheckStatus($(this).is(":checked"));
												}),
